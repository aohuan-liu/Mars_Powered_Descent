...
- `convex_opt/forward_sim.m`：与求解器一致的 ZOH 精确离散前向传播
  （`forward_sim(T_seq, params, 'rk4')` 可选 RK4 连续对照）

### ④ 验证效果
跑 `main`，看 Step 2 输出：初始高度 1500 m，恒推力（=初始重力）下最终
高度明显为负、速度增大（精确值见运行输出）。**这不是 bug**——一边烧油
质量变轻，同样的推力产生的加速度变大，所以加速下坠。再把 `mars_params.m`
里 `v0` 的 z 分量从 70 改成 -70，预测轨迹偏移方向后重新跑。
可再用 `forward_sim(T_seq, params, 'rk4')` 与默认 ZOH 结果对照，
体会"同一推力序列、不同积分器"的差异。
...
并注意输出里的**松弛紧性检查**：max|σ-‖u‖₂|≈1e-8（无损凸化紧性证据）
与原始推力界 m·σ∈[T_min, T_max]（基准下界约有 0.4 N 的 Taylor 残差，
Algorithm 1 迭代后精确满足——这是"迭代细化必要性"的量化证据）。
...
- **Eq.20-24**：连续系统离散化。取 N+1=31 个控制时刻（k=0..30）、
  ...
  X(k+1)=Ad·X(k)+Bd·[u;σ]+Bd_g，其中 A_c 幂零（A_c²=0），
  Ad=e^(A_c·Δt)=I+Δt·A_c、Bd=Δt·B_c+Δt²/2·A_c·B_c 为精确闭式。
- **Eq.25**：初末状态 + 质量下限的离散形式。
- **Eq.26-27**：推力上下界 + 常规下滑角约束的离散形式。
- **Eq.28（Problem 4）**：最终要求解的问题——min λΔtΣσ。

### ③ 对应代码（全在 `convex_opt/solve_pd_socp.m`）
- `convex_opt/zoh_matrices.m`：Ad/Bd/Bd_g 的幂零闭式（Eq.23-24）
...
- **Eq.35-36（Problem 5）**：目标 min λΔtΣσ + W_ζ·‖ζ‖₂，
  罚项逼 ζ 收敛到 0。
- **Algorithm 1**：δ=min(j,H)/H（H=3）；每轮都检查约束与 ‖ζ‖₂，
  按论文第 12 步以 ‖ζ‖₂ ≤ 10⁻⁶ 且 j≥H+1（δ=1 至少解两次）为终止条件，
  故最快第 4 轮宣布收敛。
...
| 松弛紧性 / 原始推力界 | 无损松弛应 σ≈‖u‖，T_min≤m·σ≤T_max | 基准紧性 2.75e-08、下界差 0.39 N；Algorithm 1 后紧性 9.14e-06 且精确满足 | ✅ 量化证据 |
...
2. **6-DoF Simulink 验证**（可选）：把 3-DoF 轨迹接到姿态环。
