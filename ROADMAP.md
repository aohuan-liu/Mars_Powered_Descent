...
> 学习标准：**2025 版 OCRL 为绝对标准**；2022 版笔记（`OCRL/2022cmu/`，临时参考，
> 稍后删除）仅用于评估授课内容与难度，不进入排班。
> 难度系数：1 lecture（1.5h 授课）需 **3-5h 消化**（听课 + 吸收 + 作业 + 验证，中值 4h）。
> 只排与论文相关的 lecture；**RL（L19-24）与论文无关，不安排**。
> 估算是**从零开始**的完整时长，不含 5 门复试笔试科目；阶段一已上 L1-2、HW0 进行中，
> 实际可减约 6-8h。
...
**主线合计 ≈ 183h（区间 166-202h），不含 RL / 6-DoF**
...
**软期限 1/30 小年达标**（28 个工作日，平均约 6.5h/天）；硬期限 2/4（32 个工作日）。
